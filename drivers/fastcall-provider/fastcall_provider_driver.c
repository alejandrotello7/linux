...
static long register_function(unsigned long args)
{
	...
	io_args = (struct ioctl_args *) kmalloc((sizeof(struct ioctl_args)+ sizeof((int) args->code_size)), GFP_KERNEL);
	printk(KERN_ALERT "fcp: Size of args: %lu", sizeof(args));
	printk(KERN_ALERT "fcp: Size of io_args: %lu", sizeof(io_args));
	printk(KERN_ALERT "fcp: Size of code_size: %d", (int) args->code_size);


	if (copy_from_user(io_args, (struct ioctl_args *)args, sizeof((struct ioctl_args)+args->code_size)))
		goto fail_copy;
